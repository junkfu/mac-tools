...
let arcColor: UInt32 = 0x3FD6B0      // 跳躍弧線，對應介紹頁的 AppJump 強調色
// ────────────────────────────────────────────────────────────
...
/// 由左跳到右的弧線加箭頭。
func drawJumpArc(_ ctx: CGContext, from: CGPoint, to: CGPoint, lift: CGFloat, width: CGFloat, color: CGColor) {
    let control = CGPoint(x: (from.x + to.x) / 2, y: max(from.y, to.y) + lift)
    let path = CGMutablePath()
    path.move(to: from)
    ...
    arrow.move(to: CGPoint(x: head * 0.55, y: 0))
    arrow.addLine(to: CGPoint(x: -head * 0.5, y: head * 0.62))
    arrow.addLine(to: CGPoint(x: -head * 0.5, y: -head * 0.62))
    arrow.closeSubpath()
    ctx.addPath(arrow)
...
func drawIcon(size: CGFloat) {
    ...
    if size >= 32 {
        drawGlyph("⌘", size: 420 * s, weight: .medium, color: .white,
                  center: CGPoint(x: 512 * s, y: 400 * s))
        drawJumpArc(ctx,
                    from: CGPoint(x: 318 * s, y: 690 * s),
                    to: CGPoint(x: 706 * s, y: 690 * s),
                    lift: 95 * s, width: 44 * s, color: rgb(arcColor))
    } else {
        // 16pt 的弧線只會剩下不到一個像素的灰邊，反而讓 ⌘ 變髒。
        // 跟系統圖示一樣，小尺寸換一版更簡單的：只留放大的 ⌘。
        drawGlyph("⌘", size: 560 * s, weight: .medium, color: .white,
...
echo "▶︎ 繪製各尺寸…"
"$TMP_DIR/icongen" "$TMP_DIR/AppJump.iconset"

echo "▶︎ 打包成 .icns…"
mkdir -p Resources
iconutil -c icns "$TMP_DIR/AppJump.iconset" -o Resources/AppIcon.icns

echo ""
echo "✅ 完成： $(pwd)/Resources/AppIcon.icns（$(du -h Resources/AppIcon.icns | cut -f1)）"
echo "   接著跑 ./build.sh 就會打包進 AppJump.app。"
