...
2. **`HotKeyManager`** wraps Carbon `RegisterEventHotKey` (global hotkey without Accessibility permission, default ⌘⇧X). **`HotKeyStore`** persists user customization in `UserDefaults` and converts Carbon modifiers ↔ display symbols. Factory default lives in `HotKeyDefaults` (in HotKeyManager.swift); the store overrides it. Customization UI: **`PreferencesWindowController`** (opened from the status menu) hosts **`HotKeyRecorderControl`**, a button that records the next key combo via a *local* `NSEvent` monitor — deliberately not a global monitor, so no Accessibility permission is needed; it requires at least one of ⌘/⌥/⌃ and treats bare Esc as cancel. On capture it saves to `HotKeyStore` and calls back so `AppDelegate` re-registers the hotkey and updates the menu item title.
