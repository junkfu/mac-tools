...
- `build.sh` assembles straight into `/Applications/MacCut.app` and leaves no `.app` in the repo — a second copy of the same app shows up twice in Alfred/Spotlight and confuses login items. It updates the bundle in place rather than deleting it, so the path and signature stay stable and the Screen Recording grant survives. Override the destination with `INSTALL_DIR=… ./build.sh`.
- The app icon is generated by `./make-icon.sh` (CoreGraphics → iconutil) into `Resources/AppIcon.icns`, which is checked in; a normal build doesn't need to run it.
...
- `make-dmg.sh` packages the installed `/Applications/MacCut.app` into a drag-install `.dmg` for handing to someone else; it isn't needed for local use any more, since `build.sh` already installs.
