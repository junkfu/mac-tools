...
> 代價：這把私鑰對 `codesign` 免提示，表示任何以你身分執行的程式也能用它重簽二進位、繼承 MacCut 的「螢幕錄製」授權。`build.sh` 有開 hardened runtime 擋掉 dyld 注入，私鑰也設為不可匯出；若你的機器會跑不信任的程式，改成每次 build 手動按「允許」會更安全。
