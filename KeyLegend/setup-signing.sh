...
#
# 為什麼需要：KeyLegend 要「輔助使用」權限才能在別的 App 是前景時偵測到長按 Option。macOS 的 TCC 授權綁在
# 程式碼簽章上，ad-hoc 簽章每次重編譯都會變，授權就失效、每次都要重新勾一次。
set -euo pipefail
cd "$(dirname "$0")"

IDENTITY_NAME="KeyLegend Local Signing"
...
echo ""
echo "如果你之前已經在「系統設定 → 隱私權與安全性 → 輔助使用」授權過 KeyLegend，"
